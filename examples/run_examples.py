        ...
        if exit_code == 0:
            for assertion in case.asserts:
                ...
                if proc.returncode != 0:
                    ...
                    break
                combined = f"{out or ''}\n{err or ''}"
                missing = [s for s in assertion.expect_contains if s not in combined]
                if missing:
                    exit_code = 3
                    failed_cmd = assertion.cmd
                    failed_assert = f"missing expected output: {', '.join(missing)}"
...
def _match_case(group: str, name: str, pattern: str) -> bool:
    return fnmatch.fnmatch(name, pattern) or fnmatch.fnmatch(f"{group}/{name}", pattern)


def run_with_config(cfg: RunConfig, selected_cases: list[FlatCase]) -> None:
    emitted_groups: set[str] = set()

    def on_case_done(case: FlatCase, result: RunResult, out: str, err: str) -> None:
        ...
        if result.expected_fail:
            if result.exit_code != 0:
                CONSOLE.print(
                    f"  - [yellow]XFAIL[/yellow] {case.name} ({result.expected_fail})"
                )
                return
            CONSOLE.print(f"  - [red]XPASS[/red] {case.name} ({result.expected_fail})")
            return
        if result.exit_code == 0:
            CONSOLE.print(f"  - [green]PASS[/green] {case.name}")
            return
        CONSOLE.print(f"  - [red]FAIL[/red] {case.name}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "run":
        ...
        run_with_config(cfg, selected_cases)
        return 0
    return 2
